...
import static com.almasb.fxgl.app.FXGL.getLocalizedString;
...
public class RiskMenu extends FXGLMenu {
    private ParticleSystem particleSystem;

    public RiskMenu(GameApplication app, MenuType type) {
        super(app, type);
    ...
    protected MenuBox createNewGameMenu() {
        log.debug("createNewGameMenu");

        MenuBox newGameMenu = new MenuBox();
        RiskConfig config = app.getGameConfig();

        ChoiceBox<String> player1Agent =
                new FXGLChoiceBox<>(FXCollections.observableArrayList(Constants.agents));
        player1Agent.setValue(config.getPlayer1Agent());
        ChoiceBox<String> player2Agent =
                new FXGLChoiceBox<>(FXCollections.observableArrayList(Constants.agents));
        player2Agent.setValue(config.getPlayer2Agent());

        MenuButton itemStartGame = new MenuButton("menu.newGame");
        itemStartGame.setOnAction(e -> {
            config.setPlayer1Agent(player1Agent.getValue());
            config.setPlayer2Agent(player2Agent.getValue());
            fireNewGame();
        });

        newGameMenu.add(itemStartGame);
        newGameMenu.add(player1Agent);
        newGameMenu.add(player2Agent);
